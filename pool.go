...
type channelPool struct {
	mu           sync.RWMutex
	conns        chan *idleConn              // 用于存储空闲连接的通道。
	factory      func() (interface{}, error) // 用于创建新连接的方法。
	close        func(interface{}) error
	ping         func(interface{}) error
	idleTimeout  time.Duration  // 定义空闲连接的最大存活时间。
	waitTimeOut  time.Duration  // 定义等待获取连接的最大时间。
	maxActive    int            // 连接池中允许的最大活动连接数，包括正在使用的连接和空闲连接。
	openingConns int            // 当前正在创建的新连接数。
	connReqs     []chan connReq // 表示当前正在等待分配连接的请求队列，每个请求被表示为一个 chan connReq。
}
...
func (c *channelPool) Get() (interface{}, error) {
	...
	for {
		...
		default:
			c.mu.Lock()
			log.Println("openConn %v %v", c.openingConns, c.maxActive)
